...
def build_pdf(
    *,
    ...
    pdf.cell(
        0,
        10,
        "batchlense — production quality report",
        new_x=XPos.LMARGIN,
        new_y=YPos.NEXT,
    )
    pdf.set_font("Helvetica", "", 11)
    pdf.cell(0, 8, "Max Wollenberg", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.set_text_color(0, 0, 200)
    pdf.cell(0, 8, LINKEDIN, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.set_text_color(0, 0, 0)
    pdf.ln(4)

    pdf.set_font("Helvetica", "B", 12)
    pdf.cell(0, 8, "Summary KPIs", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.set_font("Helvetica", "", 10)
    pdf.cell(
        0,
        6,
        f"Total batches: {kpis.get('n_batches', 0)}",
        new_x=XPos.LMARGIN,
        new_y=YPos.NEXT,
    )
    pdf.cell(
        0,
        6,
        f"Outliers: {kpis.get('n_outliers', 0)}",
        new_x=XPos.LMARGIN,
        new_y=YPos.NEXT,
    )
    pdf.cell(
        0,
        6,
        f"Outlier rate: {kpis.get('outlier_rate_pct', 0):.2f}%",
        new_x=XPos.LMARGIN,
        new_y=YPos.NEXT,
    )
    pdf.cell(
    ...
    pdf.set_font("Helvetica", "B", 12)
    pdf.cell(0, 8, "Outlier list", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    if len(sub) > 50:
        pdf.cell(
            0,
            6,
            f"... and {len(sub) - 50} more rows (see CSV export).",
            new_x=XPos.LMARGIN,
            new_y=YPos.NEXT,
        )
